...
//			return new TubeSection(maxLength);
	...
	public class LevelSectionList implements Comparable<LevelSectionList>, Iterable<LevelSection>{
		...
		{
			//penalize two of the same section in a row
			if(sections.size() > 0 && ls.getId() == sections.get(sections.size()-1).getId())
			{
				fitness -= 2;
				
				if(sections.size() > 1 && ls.getId() == sections.get(sections.size()-2).getId())
				{
					fitness -= 10;
					
					if(sections.size() > 2 && ls.getId() == sections.get(sections.size()-3).getId())
					{
						fitness -= 20;
					}
				}
	...
	public class HillSection extends LevelSection{
		...
		public int build(int xo, int maxLength) {
			...
			if(enemies > 0) addEnemies(xo + 1, xo + length - 1, floor, enemies/4);
			...
			while (keepGoing)
			{
				...
				else
				{
					...
					else
					{
						...
						if(enemies > 0) addEnemies(xxo, xxo + l, h - 1, enemies/4);
					...
					if(h < 5) keepGoing = false;
					if(!keepGoing && coins > 0)  addCoins(xxo - 1, xxo + l + 1, h, coins);
				}
			}
...
//	}
	...
	private void addCoins(int startX, int endX, int y, int number)
	{
		...
		if(number > endX - startX) number = endX - startX;
		
		for (int i=0; i < number; i++)
		{
			int x = random.nextInt(endX - startX) + startX;
			while(getBlock(x, y - 1) == COIN)
			{
				x = random.nextInt(endX - startX) + startX;
			}
